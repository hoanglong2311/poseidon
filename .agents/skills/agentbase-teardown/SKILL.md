...
```bash
bash .Codex/skills/agentbase/scripts/discovery.sh json
```
...
Delete in this specific order to avoid dependency errors. If any script call returns an auth error (401/403) during the teardown sequence, re-authenticate with `bash .Codex/skills/agentbase/scripts/get_token.sh --force` and retry the failed call once. If the retry also fails, report the error and continue with the next deletion.
...
bash .Codex/skills/agentbase/scripts/runtime.sh endpoints delete $RUNTIME_ID $ENDPOINT_ID
```

**Phase 2 — Runtimes** (only after all custom endpoints are deleted):
```bash
bash .Codex/skills/agentbase/scripts/runtime.sh delete $RUNTIME_ID
```

**Phase 3 — Auth providers** (API Key, Delegated, OAuth2):
```bash
# API Key providers
bash .Codex/skills/agentbase/scripts/auth.sh apikey delete --name $NAME

# Delegated providers
bash .Codex/skills/agentbase/scripts/auth.sh delegated delete --name $NAME

# OAuth2 providers
bash .Codex/skills/agentbase/scripts/auth.sh oauth2 delete --name $NAME
```

**Phase 4 — Agent identity:**
```bash
bash .Codex/skills/agentbase/scripts/identity.sh delete $NAME
```

**Phase 5 — Memory:**
```bash
bash .Codex/skills/agentbase/scripts/memory.sh delete $MEMORY_ID
```
...
PAGE=1
while true; do
  RESULT=$(bash .Codex/skills/agentbase/scripts/cr.sh images list --name $PROJECT_NAME --page $PAGE --size 100)
  # For each image in $RESULT.data[].name:
  #   bash .Codex/skills/agentbase/scripts/cr.sh images delete --name $IMAGE_NAME
...
bash .Codex/skills/agentbase/scripts/cr.sh images list --name $PROJECT_NAME
```

**Phase 7 — AIP API keys** (optional, may be shared):
```bash
bash .Codex/skills/agentbase/scripts/aip.sh api-keys delete $KEY_NAME
```
...
4. Discover all related resources using `bash .Codex/skills/agentbase/scripts/discovery.sh json`.
