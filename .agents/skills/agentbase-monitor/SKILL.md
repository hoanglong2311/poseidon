...
bash .Codex/skills/agentbase/scripts/runtime.sh logs $RUNTIME_ID --from 0 --limit 100

# With time range and keyword search
bash .Codex/skills/agentbase/scripts/runtime.sh logs $RUNTIME_ID \
  --from 0 --limit 100 \
...
**Command**:
```bash
bash .Codex/skills/agentbase/scripts/runtime.sh endpoints logs $RUNTIME_ID $ENDPOINT_ID \
  --from 0 --limit 100
```
...
```bash
# Current metrics
bash .Codex/skills/agentbase/scripts/runtime.sh endpoints metrics $RUNTIME_ID $ENDPOINT_ID
...
**Command**:
```bash
bash .Codex/skills/agentbase/scripts/runtime.sh endpoints events $RUNTIME_ID $ENDPOINT_ID
```
...
bash .Codex/skills/agentbase/scripts/runtime.sh traces search --param key=value [--param key=value ...]

# Get a single trace by ID
bash .Codex/skills/agentbase/scripts/runtime.sh traces get $TRACE_ID [--param key=value ...]

# List available values for a trace tag key (for building filters)
bash .Codex/skills/agentbase/scripts/runtime.sh traces tag-values $TAG_KEY [--param key=value ...]
```
...
for i in $(seq 1 $MAX_POLLS); do
  RESULT=$(bash .Codex/skills/agentbase/scripts/runtime.sh logs $RUNTIME_ID --from $OFFSET --limit $LIMIT)
...
bash .Codex/skills/agentbase/scripts/runtime.sh logs $RUNTIME_ID --from 0 --limit 100 --query "error"
...
LOGS=$(bash .Codex/skills/agentbase/scripts/runtime.sh logs $RUNTIME_ID --from 0 --limit 100)
...
1. **Discover all resources** using `bash .Codex/skills/agentbase/scripts/discovery.sh`
...
- **Runtimes**: Name, status, description. For each runtime, list its endpoints with name, version, URL, and status. Fetch endpoints via `bash .Codex/skills/agentbase/scripts/runtime.sh endpoints list $RUNTIME_ID`. (Note: runtime list DTO contains `id, name, description, status, statusReason, createdAt, updatedAt` — for flavor/version details, use `bash .Codex/skills/agentbase/scripts/runtime.sh versions $RUNTIME_ID` which returns `version`, `imageUrl`, `flavorId`, `autoscaling` per version.)
...
2. **For logs/metrics/events operations:**
   a. If a runtime ID is needed and not provided, list runtimes first (`bash .Codex/skills/agentbase/scripts/runtime.sh list`) and ask the user to pick one.
   b. If an endpoint ID is needed, list endpoints for the runtime (`bash .Codex/skills/agentbase/scripts/runtime.sh endpoints list $RUNTIME_ID`) and ask the user to pick one.
...
4. **For dashboard operation:**
   ...
   b. Run `bash .Codex/skills/agentbase/scripts/discovery.sh` (or `bash .Codex/skills/agentbase/scripts/discovery.sh json` for raw JSON).
